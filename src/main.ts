...
function createApp() {
  const routes: { method: 'GET' | 'POST' | 'PUT' | 'DELETE'; path: string; handler: (req: http.IncomingMessage, res: http.ServerResponse) => void }[] = [];

  function addRoute(method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, handler: (req: http.IncomingMessage, res: http.ServerResponse) => void) {
